...
def build_and_fit_model(time, locations, Y, total_counts, dimensions, config):
    ...
    with pm.Model() as variant_model:
        ...
        # Concentration parameter for Dirichlet (controls overdispersion)
        # Higher concentration = less variance (closer to mean p_mean)
        # Lower concentration = more variance (more overdispersion)
        # Use log-scale to ensure positivity
        log_concentration = pm.Normal('log_concentration', mu=np.log(50), sigma=1.0)
        concentration = pm.math.exp(log_concentration)

        logger.info(f"  Dirichlet-Multinomial with estimated concentration parameter")

        # Dirichlet layer: adds observation-level variability around mean proportions
        # theta[i] ~ Dirichlet(concentration * p_mean[i])
        ...
        alpha_dirichlet = concentration * p_mean
