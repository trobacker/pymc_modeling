...
def build_and_fit_model(time, locations, Y, total_counts, dimensions, config):
    ...
    chains = config['modeling'].get('chains', 4)  # Default to 4 if not specified
    ...
    logger.info(f"  Chains: {chains}, Cores: {cores}, Target accept: {target_accept}")

    with pm.Model() as variant_model:
        # Hierarchical hyperpriors for intercepts (alpha)
        ...
        # Hierarchical hyperpriors for slopes (beta)
        ...
        # Convert from log-odds to mean proportions using softmax
        ...
        logger.info(f"  Hierarchical priors with hyperparameters on alpha and beta")
        logger.info(f"  This enables partial pooling across locations and clades")

        # Location-specific concentration parameters for Dirichlet (controls overdispersion)
        # Higher concentration = less variance (closer to mean p_mean)
        ...
        logger.info(f"  Dirichlet-Multinomial with location-specific concentration parameters")
        logger.info(f"  This allows uncertainty calibration to vary by location")

        # Dirichlet layer: adds observation-level variability around mean proportions
        # theta[i] ~ Dirichlet(concentration[location[i]] * p_mean[i])
        # This allows each observation to have its own proportion vector
        # that varies around the mean trajectory p_mean
        # The concentration parameter scales the Dirichlet, controlling the variance:
        #   Var(theta) ≈ p_mean * (1 - p_mean) / (concentration + 1)
        # Now concentration can differ by location, enabling heterogeneous uncertainty
        concentration_expanded = concentration[locations]
        alpha_dirichlet = concentration_expanded[:, None] * p_mean
        n_observations = Y.shape[0]
        theta = pm.Dirichlet('theta', a=alpha_dirichlet, shape=(n_observations, num_clades + 1))

        # Likelihood: multinomial distribution with Dirichlet-sampled proportions
        # This creates a Dirichlet-Multinomial observation process
        # accounting for overdispersion beyond what the multinomial alone allows
